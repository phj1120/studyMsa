...
import org.example.encryptmodule.v2.MemberRepositoryV2
...
class PhoneValidationTest {
    ...
    @Test
    fun `V1 전화번호 검증 실패시 예외가 발생해야 한다`() {
    ...
    @Test
    fun `V2 전화번호 검증 실패시 예외가 발생해야 한다`() {
        // Given: FakeExternalApiClient로 검증 실패 설정
        val fakeApiClient = FakeExternalApiClient()
        fakeApiClient.shouldValidate = false

        val mockRepository = mockk<MemberRepositoryV2>()
        val memberService = MemberServiceV2(mockRepository, fakeApiClient)
    ...
    @Test
    fun `V1 전화번호 검증 성공시 정상 처리되어야 한다`() {
    ...
    @Test
    fun `V2 전화번호 검증 성공시 정상 처리되어야 한다`() {
        // Given: FakeExternalApiClient로 검증 성공 설정
        val fakeApiClient = FakeExternalApiClient()
        fakeApiClient.shouldValidate = true
